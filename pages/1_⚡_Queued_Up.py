...
mkdwn_analysis = """
    **Source:** [Berkley Lab Queued Up v2](https://emp.lbl.gov/queues): 
    Extended Analysis on Power Plants Seeking Transmission InterconnectionAs of the 
    End of 2021. Joseph Rand, Will Gorman, Dev Millstein, Andrew Mills, Joachim Seel, 
    Ryan Wiser Lawrence Berkeley National Laboratory. February 2022.
"""
...
def app():
    ...
    st.markdown(
        """**Introduction:** This interactive dashboard is designed for visualizing U.S. electrical interconnection 
        data and trends at multiple levels (i.e., national, state, county, and ISO). The data sources is from [
        Berkley Labs](https://emp.lbl.gov/queues) and [Cartographic Boundary Files](
        https://www.census.gov/geographies/mapping-files/time-series/geo/carto-boundary-file.html) from U.S. Census 
        Bureau. Several open-source packages are used to process the data and generate the visualizations, e.g., 
        [streamlit](https://streamlit.io), [geopandas](https://geopandas.org), [leafmap](https://leafmap.org), 
    ...
    with row2_col1:
        region_select = st.sidebar.selectbox('Region:', region_list, index=default_region,
                                             help='Filter report to show the market region.')

    with row2_col2:
        qyear_select = st.sidebar.slider("Enter queue year range for trends:", min_value=2001, max_value=2021, step=1,
                                         value=2014,
    ...
    with row3_col1:
        df_chart_trend = df_trend[
            (df_trend['region'] == region_select) & (df_trend['type_clean'].isin(selected_options_ft)) & (
                    df_trend['q_year'] >= qyear_select)]
    ...
    st.subheader('Trends:')
    st.markdown(
        '- PJM and MISO have the highest volume of projects completing each phase. Total volume decreases '
        'substantially across phases for most ISOs.')
    st.markdown(
        '- Historical completion rates 2000-2015: PJM (29%), MISO (27%), ISONE (22%), CAISO (13%) and NYISO (18%). ('
        'page 30)')
    ...
    st.markdown(
        '- **Only 27%** of all projects requesting interconnection from 2000 to 2016 achieved commercial operation by '
        'year-end 2021. (see pg. 2 PJM Cost Report)')
    ...
    def_cod = df_tr1[(df_tr1['q_year'] >= 2000) & (df_tr1['q_year'] <= 2016) & (df_tr1['cod_year'] <= 2021) & (
            df_tr1['cod_year'] >= 2000)]
    ...
    st.markdown(
        '- But durations from IR to interconnection agreement (IA) have been mostly steady at 2-3 years in the last '
        'decade')
    st.markdown('- In PJM, Phase 1 takes typically 2-3 months; Phase 3 takes 20-25 months.')
    st.markdown(
        '- Many markets including PJM are embarking on queue reform whuch has been encouraged and approved by FERC ('
        '2022). This includes a clustered, **“first-ready, first-serve”** approach, size-based study deposits, '
        'and increased readiness deposits that are at risk when projects withdraw later in the study process.')
